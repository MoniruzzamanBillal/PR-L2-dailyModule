...
const createStudent = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const { error, value } = student_validation_1.studentValidationSchema.validate(studentData);
        if (error) {
            console.log(error);
            res.status(400).json({ error: error.details });
        }
        const response = yield sttudent_service_1.StudentServices.createStudentIntoDb(studentData);
        res.status(200).json({
            success: true,
...
const getAllStudent = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const result = yield sttudent_service_1.StudentServices.gettAllStudentsFromDb();
        res.status(200).json({
            success: true,
            message: "retrived all students data successfully",
            data: result,
        });
    }
    catch (error) {
